    ...
    useEffect(() => {
        ...
        setFiltro(
            ...
            personagens.filter(item => (
                item.nome.toLocaleLowerCase()).indexOf(termo.toLocaleLowerCase()) !== -1
                && item.tipo.indexOf(tipo) !== -1
            )
        )
    }, [termo, tipo])
    ...
    return (
        <div className="sw">
            <div className="sw-header">
                ...
                <button onClick={() => {setTipo('humano')}}>Humano</button>
                <button onClick={filtrarRobos}>Robo</button>
            </div>
